...
# 1) Пытаемся получить путь к degradations.py через Python в активном env
BASICS_PATH=$(python - << 'EOF'
import basicsr, inspect, os
path = os.path.join(os.path.dirname(inspect.getfile(basicsr)),
                    "data", "degradations.py")
print(path)
EOF
)

# 2) Кандидатные пути под разные варианты установки
CANDIDATES=(
  "$BASICS_PATH"
...
# 3) Патчим всё, что реально существует
